...
* Course: ECO5365(002) International Trade
* Due Date: April 14, 2025
*
* This do-file replicates equation (35) using 3SLS estimation with labor as 
* the only factor of production. We estimate the system of share equations
* for 7 sectors using the provided Excel data files.
*******************************************************************************/
...
* Merge and ensure we keep only complete observations
...
keep country year ln_p_nt share_agr share_min share_man ln_lambda_agriculture ln_lambda_mining ln_lambda_manufacturing ln_rel_labor ln_dist ln_dist_sq year_*
...
global excluded_instruments "ln_dist ln_dist_sq ln_dist_pnt ln_dist_sq_pnt"
global included_instruments "ln_rel_labor ln_p_nt"
global time_controls "yg_*"
global all_instruments "$excluded_instruments $included_instruments $time_controls"

* Define single symmetry constraint for identification
constraint drop _all
constraint 1 [share_agr]ln_lambda_mining = [share_min]ln_lambda_agriculture

* Display constraint for verification
constraint dir

* 3SLS Estimation with minimal constraints
reg3 (share_agr = ln_lambda_agriculture ln_lambda_mining ln_lambda_manufacturing ln_rel_labor ln_p_nt $time_controls) ///
     (share_min = ln_lambda_agriculture ln_lambda_mining ln_lambda_manufacturing ln_rel_labor ln_p_nt $time_controls) ///
     (share_man = ln_lambda_agriculture ln_lambda_mining ln_lambda_manufacturing ln_rel_labor ln_p_nt $time_controls), ///
     constraints(1) inst($all_instruments) small
...
test [share_agr]ln_lambda_agriculture + [share_agr]ln_lambda_mining + ///
     [share_agr]ln_lambda_manufacturing = 0

* Calculate Hansen-Sargan overidentification test manually
predict resid_agr, equation(share_agr) residuals
predict resid_min, equation(share_min) residuals
predict resid_man, equation(share_man) residuals

* First stage regressions for each endogenous variable
foreach sector in agriculture mining manufacturing {
    qui reg ln_lambda_`sector' $all_instruments
    predict xb_`sector', xb
}

* Calculate test statistic
gen hansen_moment = 0
foreach eqn in agr min man {
    foreach iv of global excluded_instruments {
        qui reg `iv' $included_instruments $time_controls
...
local df = `: word count $excluded_instruments' - 1  // Subtract 1 for the constraint
...
* Get coefficients and standard errors for each sector combination
foreach row in agr min man {
    * Write row label
    local rowname = cond("`row'"=="agr", "Agriculture", cond("`row'"=="min", "Mining", "Manufacturing"))
...
foreach sector in agr min man {
    local sectorname = cond("`sector'"=="agr", "Agriculture", cond("`sector'"=="min", "Mining", "Manufacturing"))
...
drop resid_* xb_* hansen_moment yeargroup yg_* ln_dist_pnt ln_dist_sq_pnt yhat_*
